...
module.exports.creationAgenda = catchErr(async (req, res, next) =>{
    let agendaInfo = await agenda.createAgenda({Nom : req.fields.Nom, Type : req.fields.Type, idOwner : req.UserID})

    if(agendaInfo[0] == false){
        return res.status(agendaInfo[1]).json(normaliseErr(agendaInfo[1]))
    }
...
module.exports.DeleteAgenda = catchErr(async ( req, res, next) =>{
    ...
    if(dataAgenda[0] == false ){
        return res.status(dataAgenda[1]).json(normaliseErr(dataAgenda[1]))
    }

    let dataTask = await task_organisation.destroyTask({Agenda_ID : req.fields.OrganisationID})
    if(dataTask[0] == false && dataTask[1] !== 404){
        return res.status(dataTask[1]).json(normaliseErr(dataTask[1]))
    }
    
    let dataCategory = await category.destroyCategory({agenda_ID : req.fields.OrganisationID})

    if(dataCategory[0] == false && dataCategory[1] !== 404){
        return res.status(dataCategory[1]).json(normaliseErr(dataCategory[1]))
    }

    let dataPerm = await permission.destroyPerm({Organisation_ID : req.fields.OrganisationID})

    if(dataPerm[0] == false && dataPerm[1] !== 404){
        return res.status(dataPerm[1]).json(normaliseErr(dataPerm[1]))
    }

    let DestroyInfo = await agenda.destroyAgenda({ID : req.fields.OrganisationID});
    
    if(DestroyInfo[0] == false){
        return res.status(DestroyInfo[1]).json(normaliseErr(DestroyInfo[1]))
    }
...
module.exports.updateAgenda = catchErr(async (req, res, next) =>{
    if(!req.fields.Nom || !req.fields.ID){
        return res.status(406).json(normaliseErr(406))
    }
    ...
    let agendaData = await agenda.updateAgenda({Nom : req.fields.Nom}, {ID : req.fields.ID})
    if(agendaData[0] == false){
        return res.status(agendaData[1]).json(normaliseErr(agendaData[1]))
    }
